...
function ENT:Initialize()
	self:SetModel(self.GunLabModel)
...
function ENT:Use(activator)
	...
	DarkRP.notify(activator, 0, 3, "You purchased a " .. self.WeaponName .. " for " .. DarkRP.formatMoney(cash) .. "!")
	...
	if IsValid(owner) and activator ~= owner then
		...
		if gain == 0 then
			DarkRP.notify(owner, 3, 3, DarkRP.getPhrase("you_received_x", DarkRP.formatMoney(0) .. " " .. DarkRP.getPhrase("profit"), self.WeaponName .. "(" .. DarkRP.getPhrase("gun_lab") .. ")"))
		else
			owner:addMoney(gain)
			local word = DarkRP.getPhrase("profit")
			if gain < 0 then word = DarkRP.getPhrase("loss") end
			DarkRP.notify(owner, 0, 3, DarkRP.getPhrase("you_received_x", DarkRP.formatMoney(math.abs(gain)) .. " " .. word, self.WeaponName .. " (" .. DarkRP.getPhrase("gun_lab") .. ")"))
		end
	end
...
function ENT:createGun()
	...
	gun:SetModel(self.WeaponWorldModel)
	gun:SetWeaponClass(self.WeaponClass)
