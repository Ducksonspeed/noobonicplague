...
ENT.PossibleEvents = { }
ENT.PossibleEvents[ 1 ] = { name = "Mining Event", command = "np_miningevent", minProg = ENT.MaximumProgress * 0.3, maxProg = ENT.MaximumProgress * 0.6, eventCheck = function( ) if ( SVNOOB_VARS:Get( "MiningBoostActive" )  == true ) then return true else return false end end }
ENT.PossibleEvents[ 2 ] = { name = "Printer Event", command = "np_printerevent", minProg = ENT.MaximumProgress * 0.6, maxProg = ENT.MaximumProgress * 0.8, eventCheck = function( ) if ( SVNOOB_VARS:Get( "MaxMoneyPrinters" )  >= 2 ) then return true else return false end end  }
ENT.PossibleEvents[ 3 ] = { name = "Double Drill Event", command = "np_doubledrillevent", minProg = ENT.MaximumProgress * 0.8, maxProg = ENT.MaximumProgress * 1, eventCheck = function( ) if ( SVNOOB_VARS:Get( "ThumperDrillLimit" )  >= 2 ) then return true else return false end end  }
ENT.PossibleEvents[ 4 ] = { name = "Beast Event", command = "np_beast", minProg = ENT.MaximumProgress * 1, maxProg = ENT.MaximumProgress * 1,  eventCheck = function( ) if ( #ents.FindByClass( "npc_antlionguard" ) > 0  ) then return true else return false end end }
ENT.MeterLocation = "Nexus"
...
if ( SERVER ) then
	...
	function ENT:Use( ply )
		...
		if ( isAnEvent ) then
			if not ( self.PossibleEvents[isAnEvent].eventCheck( ) ) then
				...
				local timerName = "N00BRP_EventMeter_ButtonPushFade:" .. entIndex
	...
	function ENT:Draw( flags )
		...
		local progress = math.Clamp( ( self:GetProgress( ) / self.MaximumProgress ) * 95, 5, 95 ) 
		cam.Start3D2D( camPos, camAng, 0.5 )
			draw.RoundedBox( 2, -10, -90, 20, 100, Color( 255, 255, 255, 255 ) )
			draw.RoundedBox( 2, -8, -88, 16, progress, Color( 45, 45, 175, 255 ) )
		cam.End3D2D( )
		camAng:RotateAroundAxis( camAng:Up( ), 180 )
		cam.Start3D2D( camPos, camAng, 0.3 )
			draw.SimpleText( "Event Meter", "N00BRP_EventHeader", 0, -100, Color( rShade, gShade, bShade, 255 ), TEXT_ALIGN_CENTER )
			...
			draw.SimpleText( "Current Event: " .. currentEvent, "N00BRP_SmallHeader", 0, -50, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER )
		cam.End3D2D( )
	end
end