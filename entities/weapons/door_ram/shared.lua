...
function SWEP:PrimaryAttack()
	...
	if game.GetMap() == "lair_of_the_Beast8" then return end
	...
	if ( trace.Entity:IsGodded( ) ) then c = false end
	...
	if (trace.Entity:isDoor()) then
		...
		if ( self.Owner:isCP( ) and trace.Entity:getKeysDoorGroup( ) == "Government Owned" ) then
			allowed = true
		end
	...
	elseif (trace.Entity:IsVehicle()) then
		...
		if driver and driver.ExitVehicle then
			if ( driver:getDarkRPVar( "wanted" ) ) then
				driver:ExitVehicle()
				driver.lastEjectedFromVehicle = CurTime( )
			end
		end
		local veh = trace.Entity
		veh.passengerSeats = veh.passengerSeats or { }
		if ( veh.passengerSeats and istable( veh.passengerSeats ) and #veh.passengerSeats > 0 ) then
			for index, seat in ipairs ( veh.passengerSeats ) do
				if ( IsValid( seat ) and seat:IsVehicle( ) and IsValid( seat:GetDriver( ) ) and seat:GetDriver( ):getDarkRPVar( "wanted" ) ) then
					seat:GetDriver( ):ExitVehicle( )
					seat.lastEjectedFromVehicle = CurTime( )
				end
			end
		end
