...
local function lockUnlockAnimation(ply, snd)
	...
	timer.Simple(0.1, function() if IsValid(ply) then ply:EmitSound(snd) end end)
...
function SWEP:PrimaryAttack()
	...
	if not lookingAtLockable(self.Owner, trace.Entity) then --FUCK YOU FUCK DARKP AND FUCK LIFE 
	...
	if trace.Entity:getKeysNonOwnable() then return end
	...
	elseif trace.Entity:IsVehicle() then
		DarkRP.notify(self.Owner, 1, 3, "This isn't your car!")
	else
