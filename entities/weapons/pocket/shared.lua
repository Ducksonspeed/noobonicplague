...
function SWEP:SecondaryAttack()
	...
	local item = table.Count( self.Owner:getPocketItems() )
	if item <= 0 then
		DarkRP.notify(self.Owner, 1, 4, DarkRP.getPhrase("pocket_no_items"))
		return
	end
	local keyTable = table.GetKeys( self.Owner:getPocketItems( ) )
	self.Owner:dropPocketItem( keyTable[#keyTable] )
end

function SWEP:Reload()
	if not CLIENT then return end
	--DarkRP.openPocketMenu()
	ply:ConCommand( "say /viewbank" )
end
