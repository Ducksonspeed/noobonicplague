if SERVER then
	...
	util.AddNetworkString("vip_lockpick_time")
end

if CLIENT then
	SWEP.PrintName = "VIP Lock Pick"
	SWEP.Slot = 5
	SWEP.SlotPos = 1
	SWEP.DrawAmmo = false
...
SWEP.DefaultLockPickTime = 10
SWEP.InstantLockPickCooldown = 30
...
if CLIENT then
	net.Receive("vip_lockpick_time", function()
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		local timeMulti = 1
		local instantPick = false
		local perkPoints = self.Owner:HasPerksInTree( "Lockpicking Perks" )
		if ( perkPoints ) then
			if ( perkPoints <= 4 ) then
				timeMulti = ( 1 - ( tonumber( "0." .. perkPoints ) or 0 ) )
			else
				self.nextInstantLockpick = self.nextInstantLockpick or 0
				if ( self.nextInstantLockpick < CurTime( ) ) then
					DarkRP.notify( self.Owner, 4, 4, "You've instantly lockpicked the door!" )
					instantPick = true
					self.nextInstantLockpick = CurTime( ) + self.InstantLockPickCooldown
				end
				timeMulti = 0.6
			end
		end
		...
		if ( instantPick ) then
			self.LockPickTime = 0.1
		else
			--self.LockPickTime = math.Rand( 10, 30 ) * timeMulti
			self.LockPickTime = self.DefaultLockPickTime * timeMulti
		end
		net.Start("vip_lockpick_time")
