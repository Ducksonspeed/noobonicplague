		...
		if ( mySQLControl:IsDatabaseConnected( ) ) then
			for k,v in pairs(player.GetAll()) do
				...
				mySQLControl:Query([[SELECT * FROM darkrp_player WHERE uid = ]].. UniqueID ..[[;]], function(data)
					...
					/*mySQLControl:GrabRows( "darkrp_wallets", "steam", v:EncloseSteamID( ), function( data )
						if not data or not data[1] then return end
						v:setDarkRPVar("money", data[1].amount )
					end )*/
				end)
...
function DarkRP.GenerateCitizenName( attempts, ply )
	local rndName = "Citizen " .. math.random( 0, 100 ) .. " " .. math.random( 0, 9999 )
	mySQLControl:ColumnValueExists( "darkrp_player", "rpname", sql.SQLStr( rndName ), function( data )
		if ( !data or !data[1] ) then
			--MySQLite.query([[UPDATE darkrp_player SET rpname = ]] .. MySQLite.SQLStr(rndName) .. [[ WHERE UID = ]] .. ply:UniqueID() .. ";")
			mySQLControl:Query([[UPDATE darkrp_player SET rpname = ]] .. MySQLite.SQLStr(rndName) .. [[ WHERE UID = ]] .. ply:UniqueID() .. ";", function( ) end )
			ply:setDarkRPVar( "rpname", rndName )
		else
			local attemptNumber = attempts + 1
			if ( attemptNumber > 10000 ) then return end
			DarkRP.GenerateCitizenName( attemptNumber )
		end
	end )
end
...
function DarkRP.storeRPName(ply, name)
	...
	mySQLControl:Query([[UPDATE darkrp_player SET rpname = ]] .. MySQLite.SQLStr(name) .. [[ WHERE UID = ]] .. ply:UniqueID() .. ";", function( ) end)
end
...
function DarkRP.retrieveRPNames(name, callback)
	...
	mySQLControl:Query("SELECT COUNT(*) AS count FROM darkrp_player WHERE rpname = "..MySQLite.SQLStr(name)..";", function(r)
...
function DarkRP.retrievePlayerData(ply, callback, failed, attempts)
	...
	mySQLControl:Query(string.format([[REPLACE INTO playerinformation VALUES(%s, %s);]], MySQLite.SQLStr(ply:UniqueID()), MySQLite.SQLStr(ply:SteamID())), function( ) end )
	--MySQLite.query("SELECT rpname, wallet, salary FROM darkrp_player WHERE uid = " .. ply:UniqueID() .. ";", callback, function()
	mySQLControl:Query("SELECT rpname, wallet, salary FROM darkrp_player WHERE uid = " .. ply:UniqueID() .. ";", function( data )
		if ( #data > 0 ) then
			callback( data )
		else
...
function DarkRP.createPlayerData(ply, name, wallet, salary)
	...
	mySQLControl:Query([[INSERT INTO darkrp_player VALUES(]] ..
			ply:UniqueID() .. [[, ]] ..
			MySQLite.SQLStr(name)  .. [[, ]] ..
			salary  .. [[, ]] ..
			wallet .. ");", function( ) end )
	DarkRP.GenerateCitizenName( 0, ply )
end
...
function DarkRP.storeMoney(ply, amount)
	...
	mySQLControl:Query( [[UPDATE darkrp_player SET wallet = ]] .. amount .. [[ WHERE uid = ]] .. ply:UniqueID(), function( ) end )
	//mySQLControl:UpdateRow( "darkrp_wallets", { "amount = " .. amount }, { "steam = " .. ply:EncloseSteamID( ) } )
end
...
local function resetAllMoney(ply,cmd,args)
	...
	mySQLControl:Query( "UPDATE darkrp_player SET wallet = "..GAMEMODE.Config.startingmoney.." ;", function( ) end )
	for k,v in pairs(player.GetAll()) do
...
function DarkRP.storeSalary(ply, amount)
	...
	mySQLControl:Query( [[UPDATE darkrp_player SET salary = ]] .. amount .. [[ WHERE uid = ]] .. ply:UniqueID(), function( ) end )
	return amount
end
...
function meta:restorePlayerData()
	...
	DarkRP.retrievePlayerData(self, function(data)
		...
		/*mySQLControl:Query( "SELECT * FROM darkrp_wallets WHERE steam = "..self:EncloseSteamID()..";", function( data )
			if ( #data == 0 ) then
				mySQLControl:Query( "INSERT INTO darkrp_wallets VALUES( "..self:EncloseSteamID()..", "..info.wallet.." );" );
			end
		...
		/*
		mySQLControl:GrabRows( "darkrp_wallets", "steam", self:EncloseSteamID( ), function( data )
			if ( !data or !data[1] ) then
				mySQLControl:InsertInto( "darkrp_wallets", { self:EncloseSteamID( ), info.wallet } )
				self:setDarkRPVar("money", tonumber( data[1].amount ) )
			end
		end )
		*/
		...
		DarkRP.createPlayerData( self, math.random( 100000, 500000 ), GAMEMODE.Config.startingmoney, GAMEMODE.Config.normalsalary )
		error("Failed to retrieve player information from MySQL server, attempting to create new.")
	end)
end
...
function DarkRP.storeDoorData(ent)
	...
	MySQLite.query([[REPLACE INTO darkrp_door (idx, map, isLocked, isDisabled) VALUES(]]..
		ent:doorIndex() ..[[, ]] ..
		MySQLite.SQLStr(map) ..
