...
function GM:UpdatePlayerSpeed(ply)
	...
	hook.Call( "N00BRP_ModifyPlayerSpeed", { }, ply, ply:isArrested( ), ply:isCP( ) )
end
...
local function calcPlyCanHearPlayerVoice(listener)
	...
	for _, talker in pairs( player.GetAll() ) do
		listener.DrpCanHear[ talker ] = !vrad or ( listener:GetShootPos():DistToSqr( talker:GetShootPos() ) < 302500 and ( !dynv or IsInRoom( listener, talker ) ) );
	end
end
...
function GM:PlayerCanHearPlayersVoice(listener, talker)
	if ( listener.mutedPlayerList and listener.mutedPlayerList[talker:SteamID( )] ) then return false end
	if ( listener.RadioChannel and talker.RadioChannel and listener.RadioChannel == talker.RadioChannel ) then return true; end

	local canHear = listener.DrpCanHear and listener.DrpCanHear[talker]
	return canHear, bIs3D;
end
...
function GM:PlayerDeath(ply, weapon, killer)
	...
	if killer == ply then
		DarkRP.log(ply:Nick() .. " committed suicide", Color(255, 190, 0))
	else
...
function GM:PlayerSelectSpawn(ply)
	...
	local customSpawnPoints = NOOBRP:GetTeamSpawns( ply:Team( ) )
	if ( !ply:isArrested( ) and customSpawnPoints and istable( customSpawnPoints ) and #customSpawnPoints > 0 ) then
		POS = customSpawnPoints[math.random(#customSpawnPoints)]
	end
	...
	if ( ply.spawnPosOverride ) then
		POS = ply.spawnPosOverride
	end
...
function GM:PlayerSpawn(ply)
	...
	if GAMEMODE.Config.babygod and not ply.IsSleeping and not ply.Babygod then
		...
		//if not ( ply:getDarkRPVar( "IsGhost" ) ) then
	...
	hook.Call( "N00BRP_ModifyPlayerHealth", { }, ply, GAMEMODE.Config.startinghealth or 100 )
	hook.Call( "N00BRP_ModifyPlayerSpeed", { }, ply, ply:isArrested( ), ply:isCP( ) )
	local _, pos = self:PlayerSelectSpawn(ply)
	ply:SetPos(pos)
	hook.Call( "N00BRP_OnDarkRPPlayerSpawn", { }, ply, pos )
	...
	for k, v in pairs(ents.GetAll()) do
		...
		for _, customEnt in pairs(DarkRPEntities) do
			if class == customEnt.ent and v.SID == ply.SID then
				...
				for index, whitelistEnt in ipairs ( entityWhitelist ) do
					if ( class == whitelistEnt ) then
						isWhitelisted = true
						break
					end
				end
