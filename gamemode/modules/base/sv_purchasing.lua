...
local function BuyAmmo(ply, args)
	...
	ply:addMoney(-cost)
	ply:GiveAmmo( found.amountGiven, found.ammoType, false )
	/*local trace = {}
	trace.start = ply:EyePos()
	trace.endpos = trace.start + ply:GetAimVector() * 85
	trace.filter = ply
...
local function SetPrice(ply, args)
	...
	local base = tr.Entity.Base
	if IsValid(tr.Entity) and (class == "gunlab" or class == "microwave" or class == "drug_lab" or base == "base_gunlab" ) and tr.Entity.SID == ply.SID then
