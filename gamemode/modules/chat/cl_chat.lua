...
local function AddToChat(bits)
	...
	if ( ply.getDarkRPVar and ply:getDarkRPVar( "IsDisguised" ) ) then
		col1 = team.GetColor( ply:getDarkRPVar( "IsDisguised" ) )
	end
	...
	local plyTitle = ply:getDarkRPVar( "TitleEnum" )
	local titleData = nil
	if ( plyTitle ) then
		titleData = { }
		titleData.col = NOOBRP.Titles[ tonumber( plyTitle ) ].col
		titleData.title = NOOBRP.Titles[ tonumber( plyTitle) ].title
	end
	local shouldShow
		...
		if shouldShow ~= true then
			if ( rank == "superadmin" ) then
				if not ( titleData ) then
					chat.AddNonParsedText( col1, prefixText, Color( 175, 45, 45 ), " (ADMIN)", col2, ": "..text )
				else
					chat.AddNonParsedText( col1, prefixText, titleData.col, " <" .. titleData.title .. ">", Color( 175, 45, 45 ), " (ADMIN)", col2, ": "..text )
				end
			elseif ( rank == "admin" ) then
				if not ( titleData ) then
					chat.AddNonParsedText( col1, prefixText, Color( 45, 175, 45 ), " (ADMIN)", col2, ": "..text )
				else
					chat.AddNonParsedText( col1, prefixText, titleData.col, " <" .. titleData.title .. ">", Color( 45, 175, 45 ), " (ADMIN)", col2, ": "..text )
				end
			else
				if not ( titleData ) then
					chat.AddNonParsedText(col1, prefixText, col2, ": "..text)
				else
					chat.AddNonParsedText(col1, prefixText, titleData.col, " <" .. titleData.title .. ">", col2, ": "..text)
				end
			end
		end
	else
		if not ( tobool( GetConVarNumber( "noobrp_enablecustomchatbox" ) ) ) then
