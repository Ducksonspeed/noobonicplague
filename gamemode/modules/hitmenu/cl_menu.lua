...
function PANEL:AddPlayerRows()
	...
	if ( LocalPlayer( ).revengeTable ) then
		for index, ply in ipairs ( player.GetAll( ) ) do
			for uniqueid, amt in pairs ( LocalPlayer( ).revengeTable ) do
				if ( tonumber( ply:SafeUniqueID( ) ) == tonumber( uniqueid ) and tonumber( LocalPlayer( ):SafeUniqueID( ) ) ~= tonumber( uniqueid ) and !ply:getDarkRPVar( "IsAFK" ) ) then
