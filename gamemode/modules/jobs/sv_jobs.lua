...
local function FinishDemote(vote, choice)
	...
	if choice == 1 then
		...
		NOOB_ADMIN:NotifyAdmins( target:Nick() .. "( " .. target:UserID() .. ") was successfully demoted for: \n" .. vote.info.reason, NP_ADMIN_ATTENTION )
	else
...
local function Demote(ply, args)
	...
	p.CurrentDemote = p.CurrentDemote or { }
	p.CurrentDemote.Players = p.CurrentDemote.Players or { }
	p.CurrentDemote.Count = p.CurrentDemote.Count or 0
	p.CurrentDemote.PlayerStarted = p.CurrentDemote.PlayerStarted or nil
	p.CurrentDemote.Reason = p.CurrentDemote.Reason or ""
	local demoteAmount = SVNOOB_VARS:Get( "DemotesTilVoteCount", true, "number", 5 )
	if ( IsValid( p ) and p.CurrentDemote.Active ) then
		if not ( p.CurrentDemote.Players[ ply:SteamID( ) ] ) then
			if ( p.CurrentDemote.Count < ( demoteAmount - 1 ) ) then
				p.CurrentDemote.Players[ply:SteamID( )] = true
				p.CurrentDemote.Count = p.CurrentDemote.Count + 1
				DarkRP.notify( ply, 1, 4, ( demoteAmount - p.CurrentDemote.Count ) .. " more demotes are required to start a vote." )
				return ""
			end
		else
			DarkRP.notify( ply, 1, 4, ( demoteAmount - p.CurrentDemote.Count ) .. " more demotes are required to start a vote." )
	...
	if ( #tableargs == 1 and p.CurrentDemote.Count ~= 4 ) then
	...
	if p then
		...
		if not RPExtraTeams[p:Team()] or RPExtraTeams[p:Team()].candemote == false then
			...
			if not ( p.CurrentDemote.Active ) then
				p.CurrentDemote.Active = true
				p.CurrentDemote.Players[ply:SteamID( )] = true
				p.CurrentDemote.Count = 1
				p.CurrentDemote.PlayerStarted = ply
				p.CurrentDemote.Reason = reason
				DarkRP.notifyAll( NOTIFY_HINT, 5, ply:Name( ) .. " is demoting " .. p:Name( ) .. " for: " .. reason .. "\nType /demote " .. p:UserID( ) .. " to contribute towards a vote." )
				local amtLeft = SVNOOB_VARS:Get( "DemotesTilVoteCount", true, "number", 5 ) - 1
				DarkRP.notify( ply, 1, 4, amtLeft .. " more demotes are required to start a vote." )
				local expireTime = SVNOOB_VARS:Get( "DemoteExpireTime", true, "number", 30 )
				timer.Simple( expireTime, function( )
					if not ( IsValid( p ) ) then return end
					p.CurrentDemote.Active = false
					p.CurrentDemote.Players = { }
					p.CurrentDemote.Count = 0
					p.CurrentDemote.PlayerStarted = nil
					p.CurrentDemote.Reason = ""
				end )
				return ""
			end
			if not ( IsValid( p.CurrentDemote.PlayerStarted ) ) then
				p.CurrentDemote.Active = false
				p.CurrentDemote.Players = { }
				p.CurrentDemote.Count = 0
				p.CurrentDemote.PlayerStarted = nil
			...
			DarkRP.talkToPerson(p, team.GetColor(p.CurrentDemote.PlayerStarted:Team()), DarkRP.getPhrase("demote") .. " " ..p.CurrentDemote.PlayerStarted:Nick(),Color(255,0,0,255), DarkRP.getPhrase("i_want_to_demote_you", p.CurrentDemote.Reason), p)
			DarkRP.notifyAll(0, 4, DarkRP.getPhrase("demote_vote_started", p.CurrentDemote.PlayerStarted:Nick(), p:Nick()))
			DarkRP.log(DarkRP.getPhrase("demoting", string.format("%s(%s)", p.CurrentDemote.PlayerStarted:Nick(), p.CurrentDemote.PlayerStarted:SteamID()), string.format("%s(%s)[%s]: %s", p:Nick(), p:SteamID(), team.GetName(p:Team()), p.CurrentDemote.Reason)), Color(255, 128, 255, 255))
			p.IsBeingDemoted = true
			DarkRP.createVote(p:Nick() .. ":\n"..DarkRP.getPhrase("demote_vote_text", p.CurrentDemote.Reason), "demote", p, 20, FinishDemote,
			{
				[p] = true,
				[p.CurrentDemote.PlayerStarted] = true
			}, function(vote)
				if not IsValid(vote.target) then return end
				vote.target.IsBeingDemoted = nil
			end, {source = p.CurrentDemote.PlayerStarted, reason = p.CurrentDemote.Reason})
