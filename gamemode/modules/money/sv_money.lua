...
function meta:payDay()
	...
	if not self:isArrested() then
		amount = math.floor( GAMEMODE.Config.normalsalary )
		if ( RPExtraTeams[self:Team( )] and RPExtraTeams[self:Team( )].salary ) then
			amount = RPExtraTeams[self:Team( )].salary
		end
...
local function GiveMoney(ply, args)
	...
	if IsValid(trace.Entity) and trace.Entity:IsPlayer() and trace.Entity:GetPos():Distance(ply:GetPos()) < 150 then
		if ( trace.Entity:IsGhost() ) then
			DarkRP.notify( ply, 1, 4, "A  ghost is infront of you, use /trade (recommended) or /transfer on the specific player." );
			return "";
		end
