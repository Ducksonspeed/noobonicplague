...
function plyMeta:warrant(warranter, reason)
	...
	local centerMessage = DarkRP.getPhrase("warrant_approved", self:Nick(), warranterNick)
	local printMessage = DarkRP.getPhrase("warrant_ordered", warranterNick, self:Nick() )
...
function plyMeta:requestWarrant(suspect, actor, reason)
	...
	local question = DarkRP.getPhrase("warrant_request", actor:Nick(), suspect:Nick() )
	DarkRP.createQuestion(question, suspect:EntIndex() .. "warrant", self, 40, finishWarrantRequest, actor, suspect )
end
...
function plyMeta:wanted(actor, reason)
	...
	local centerMessage = DarkRP.getPhrase("wanted_by_police", self:Nick(), actorNick)
	local printMessage = DarkRP.getPhrase("wanted_by_police_print", actorNick, self:Nick() )
	...
	//PrintMessage( HUD_PRINTCENTER, self:Nick().." is wanted by the police!" );
