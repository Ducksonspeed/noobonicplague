...
function DarkRP.addJailPos(pos)
	...
	table.insert(NOOBRP.JailPoses, pos)

	mySQLControl:Query( "INSERT INTO darkrp_position (map, type, x, y, z) VALUES( " .. SQLStr( game.GetMap( ) ) .. ", 'J', " .. pos[1] .. ", " .. pos[2] .. ", " .. pos[3] .. " );", function( ) end )
	/*local insQuery = "INSERT INTO darkrp_position(map, type, x, y, z) VALUES(%s, 'J', %s, %s, %s);"
	insQuery = string.format(insQuery, map, strPos[1], strPos[2], strPos[3])
...
function DarkRP.retrieveJailPos()
	...
	if ( !NOOBRP.JailPoses or #NOOBRP.JailPoses < 1 ) then return Vector( 0, 0, 0 ) end
	local oldestPos = NOOBRP.JailPoses[JailIndex]
	JailIndex = JailIndex % #NOOBRP.JailPoses + 1
...
function DarkRP.jailPosCount()
	return table.Count(NOOBRP.JailPoses or {})
end
...
function DarkRP.addTeamSpawnPos(t, pos)
	...
	//mySQLControl:Query("DELETE FROM darkrp_position WHERE map = " .. SQLStr( map ) .. " AND id IN ( SELECT id FROM darkrp_jobspawn WHERE team = " .. t .. ");", function( ) end )
	mySQLControl:Query( "INSERT INTO darkrp_position(map, type, x, y, z) VALUES( " .. SQLStr( map ) .. ", 'T', " .. pos[1] .. ", " .. pos[2] .. ", " .. pos[3] .. ");", function( ) 
		mySQLControl:Query( "SELECT MAX(id) AS id FROM darkrp_position WHERE map = " .. SQLStr( map ) .. " AND type = 'T';", function( data )
			if ( data and #data > 0 and data[1] ) then
				mySQLControl:Query( "INSERT INTO darkrp_jobspawn VALUES( " .. data[1].id .. ", " .. t .. ");", function( ) end )
				NOOBRP.TeamSpawns[t] = NOOBRP.TeamSpawns[t] or { }
				table.insert( NOOBRP.TeamSpawns[t], Vector( pos[1], pos[2], pos[3] ) )
				//table.insert( NOOBRP.TeamSpawns, { id = id, map = map, x = pos[1], y = pos[2], z = pos[3], team = t } )
			end
		end )
	end )
	/*MySQLite.query([[INSERT INTO darkrp_position(map, type, x, y, z) VALUES(]] .. MySQLite.SQLStr(map) .. [[, "T", ]] .. pos[1] .. [[, ]] .. pos[2] .. [[, ]] .. pos[3] .. [[);]]
